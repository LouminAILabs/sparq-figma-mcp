// This is the main code file for the SPARQ Figma MCP plugin
// It handles Figma API commands via secure embedded bridge (no network exposure)

// Plugin state - NO PORTS OR WEBSOCKETS
const state = {
  embeddedBridgeActive: false,
  sessionId: null,
};

// Initialize secure embedded bridge
function initializeEmbeddedBridge() {
  state.sessionId = generateSessionId();
  state.embeddedBridgeActive = true;
  
  console.log('🔒 SPARQ Embedded Bridge initialized - Zero network exposure');
  
  // Notify UI that embedded bridge is ready
  figma.ui.postMessage({
    type: 'embedded-bridge-ready',
    sessionId: state.sessionId,
    message: 'Secure embedded communication established'
  });
}

// Generate unique session ID for secure communication
...
function sendProgressUpdate(
  ...
  // Send to UI via secure embedded bridge
...
// Show UI with embedded bridge
figma.showUI(__html__, { width: 350, height: 600 });

// Initialize embedded bridge when plugin starts
initializeEmbeddedBridge();

// Plugin commands from UI via embedded bridge
figma.ui.onmessage = async (msg) => {
  ...
  switch (msg.type) {
    case "embedded-bridge-connect":
      // Handle embedded bridge connection request
      initializeEmbeddedBridge();
      break;
    ...
    case "execute-command":
      // Execute commands received from UI via embedded bridge
      try {
        const result = await handleCommand(msg.command, msg.params);
        // Send result back to UI via embedded bridge
        figma.ui.postMessage({
          type: "command-result",
          id: msg.id,
          result,
...
figma.on("run", ({ command }) => {
  console.log(`🔒 SPARQ Plugin command: ${command}`);
  
  if (command === "connect") {
    // Initialize plugin when user selects "Connect to Cursor AI" from menu
    initializeEmbeddedBridge();
    figma.ui.postMessage({ 
      type: "auto-connect-embedded-bridge",
      sessionId: state.sessionId
    });
  }
});

// Update plugin settings - NO PORT SETTINGS
function updateSettings(settings) {
  // Store secure settings (no ports or network config)
  const secureSettings = {
    embeddedBridgeActive: state.embeddedBridgeActive,
    sessionId: state.sessionId,
    lastConnected: Date.now(),
  };

  figma.clientStorage.setAsync("settings", secureSettings);
  
  console.log('🔒 Settings updated - Embedded bridge only');
}
...
(async function initializePlugin() {
  try {
    ...
    if (savedSettings) {
      if (savedSettings.embeddedBridgeActive) {
        state.embeddedBridgeActive = savedSettings.embeddedBridgeActive;
      }
      if (savedSettings.sessionId) {
        state.sessionId = savedSettings.sessionId;
      }
    }
