...
import { secureBridge } from "../embedded-communication.js";
...
// Secure embedded communication state
...
let isEmbeddedBridgeConnected = false;
...
// Secure bridge configuration - no external ports needed
const BRIDGE_CONFIG = {
  maxConnections: 10,
  timeoutMs: 30000,
  retryAttempts: 3,
...
// Design strategy prompt removed - server.prompt() is not valid in MCP SDK

// Read design strategy prompt removed - server.prompt() is not valid in MCP SDK
...
// Text Replacement Strategy prompt removed - server.prompt() is not valid in MCP SDK
...
// Annotation Conversion Strategy prompt removed - server.prompt() is not valid in MCP SDK

// Instance Slot Filling Strategy prompt removed - server.prompt() is not valid in MCP SDK
...
// Reaction to connector strategy prompt removed - server.prompt() is not valid in MCP SDK
...
// Initialize secure embedded bridge (replaces WebSocket connection)
async function initializeSecureBridge(): Promise<boolean> {
  try {
    if (isEmbeddedBridgeConnected) {
      logger.info('✅ Secure bridge already initialized');
      return true;
    }

    // Start the embedded communication bridge
    await secureBridge.start();
    
    // Set up event listeners for bridge messages
    secureBridge.on('channelMessage', (data) => {
      try {
    ...
    return;
        ...
        if (message.payload && message.payload.id && pendingRequests.has(message.payload.id)) {
          const request = pendingRequests.get(message.payload.id)!;
          clearTimeout(request.timeout);

          if (message.payload.error) {
            logger.error(`Error from Figma: ${message.payload.error}`);
          ...
          logger.debug(`Received broadcast in channel ${channelName}: ${JSON.stringify(message)}`);
        }
      } catch (error) {
        logger.error(`Error processing bridge message: ${error instanceof Error ? error.message : String(error)}`);
      }
    });

    secureBridge.on('participantJoined', ({ participantId, channelName }) => {
      logger.info(`✅ Participant ${participantId} joined channel ${channelName}`);
  });

    secureBridge.on('participantLeft', ({ participantId, reason }) => {
      logger.info(`👋 Participant ${participantId} left: ${reason}`);
    });

    secureBridge.on('started', () => {
      isEmbeddedBridgeConnected = true;
      logger.info('🔒 Secure embedded bridge started successfully');
      logger.info('🚀 Zero network exposure - Using secure IPC communication');
    });

    secureBridge.on('stopped', () => {
      isEmbeddedBridgeConnected = false;
      logger.info('🛑 Secure embedded bridge stopped');
      
      // Reject all pending requests on bridge stop
      for (const [id, request] of pendingRequests.entries()) {
        clearTimeout(request.timeout);
        request.reject(new Error("Secure bridge stopped"));
    ...
    logger.error(`Failed to initialize secure bridge: ${error instanceof Error ? error.message : String(error)}`);
    isEmbeddedBridgeConnected = false;
    return false;
  }
}

// Handle progress updates from Figma
function handleProgressUpdate(message: any): void {
  try {
    const progressData = message.payload as CommandProgressUpdate;
    const requestId = message.id || '';
      ...
      // For completed updates, log completion
          if (progressData.status === 'completed' && progressData.progress === 100) {
            logger.info(`Operation ${progressData.commandType} completed, waiting for final result`);
          }
        }
  } catch (error) {
    logger.error(`Error handling progress update: ${error instanceof Error ? error.message : String(error)}`);
  }
}

// Function to join a channel using secure bridge
async function joinChannel(channelName: string): Promise<void> {
  try {
    // Ensure bridge is initialized
    if (!isEmbeddedBridgeConnected) {
      const initialized = await initializeSecureBridge();
      if (!initialized) {
        throw new Error("Failed to initialize secure communication bridge");
      }
    }

    // Join channel using secure bridge
    const participantId = `mcp-server-${uuidv4()}`;
    const result = await secureBridge.joinChannel(participantId, channelName);
    ...
    logger.info(`✅ Joined secure channel: ${channelName}`);
...
// Function to send commands to Figma using secure bridge
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!isEmbeddedBridgeConnected) {
        const initialized = await initializeSecureBridge();
        if (!initialized) {
          reject(new Error("Failed to initialize secure communication bridge"));
      return;
        }
    }
    ...
    pendingRequests.set(id, {
      ...
      // Send command through secure bridge
      if (command === "join") {
        // Handle join command specially
        try {
          await joinChannel((params as any).channel);
          resolve({ success: true, message: `Joined channel: ${(params as any).channel}` });
          pendingRequests.delete(id);
        ...
        // Send regular command through bridge
        const result = await secureBridge.sendMessage(participantId, currentChannel!, messagePayload);
        ...
        logger.info(`📤 Sent secure command to Figma: ${command}`);
...
async function main() {
  try {
    // Initialize secure embedded bridge (replaces WebSocket connection)
    logger.info('🚀 Starting SPARQ Figma MCP Server with embedded security...');
    const bridgeInitialized = await initializeSecureBridge();
    
    if (bridgeInitialized) {
      logger.info('🔒 Secure communication bridge ready');
      logger.info('🛡️ Zero network exposure - Maximum security');
    } else {
      logger.warn('⚠️ Bridge initialization failed - Will retry on first command');
    }
  } catch (error) {
    logger.warn(`Could not initialize secure bridge initially: ${error instanceof Error ? error.message : String(error)}`);
    logger.warn('Will try to initialize when the first command is sent');
  }

  // Start the MCP server with stdio transport
  const transport = new StdioServerTransport();
  await server.connect(transport);
  logger.info('✅ SPARQ-FigmaMCP server running on stdio with embedded security');
  logger.info('📋 Available commands: get_document_info, join_channel, and 40+ design tools');
}

// Export for testing
export { server, initializeSecureBridge, sendCommandToFigma, joinChannel, main };

// Only run the server if not in test mode
if (process.env.NODE_ENV !== 'test' && !process.env.JEST_WORKER_ID && !process.env.VITEST && !process.env.BUN_TEST) {
  main().catch(error => {
